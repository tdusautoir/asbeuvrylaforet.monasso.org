...
if (isset($_GET["idLicencie"]) && !empty($_GET["idLicencie"])) {
    ...
    if (filter_var($idLicencie, FILTER_VALIDATE_INT)) {
        $info = $db->prepare("SELECT licencie.prenom FROM licencie WHERE licencie.idLicencie = $idLicencie");
        ...
        if ($result) {
            create_flash_message("delete_success", "Le licencié « $firstname_licencie » a bien été supprimé.", FLASH_SUCCESS);
