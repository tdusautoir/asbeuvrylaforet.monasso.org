            ...
            if (!empty($_POST["password"]) && isset($_POST["password"])) {
                if (!empty($_POST["password_verif"]) && isset($_POST["password_verif"])) {
                    ...
                    if ($utilisateur_admin) {  //$utilisateur_admin = true donc utilisateur trouvé en tant que admin
                        ...
                        if ($utilisateur_educ) {
                            $dateToken = strtotime('+10 minutes', strtotime($utilisateur_educ["date_cr_token"]));
                            if ($dateToday < $dateToken) {
                                if ($_POST["password"] == $_POST["password_verif"]) {
                                    $password = password_hash($_POST["password"], PASSWORD_DEFAULT);

                                    $pw_update = $db->prepare('UPDATE educ SET password = ?, pw_recup_token = NULL WHERE pw_recup_token = ?');
